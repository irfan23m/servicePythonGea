        ...
        for slot in self.required_slots(tracker):
            if self._should_request_slot(tracker, slot):
                ...
                if slot == 'kendaraan':
                    ...
                    buttons = []
                    for i in result:
                        buttons.append({'title': f'{str(i[0])} gram', 'payload': f'b- {str(i[0])}'})
                    dispatcher.utter_message(template='utter_ask_berat_emasku', buttons=buttons, **tracker.slots)
                    return [SlotSet(REQUESTED_SLOT, slot)]
                elif slot == 'tenor_emasku':
                    max_tenor = 12
                    buttons = []
                    for i in range(1,max_tenor+1):
                        buttons.append({'title': f'{i} bulan', 'payload': f'te- {i}'})
                    dispatcher.utter_message(template='utter_ask_tenor_emasku', buttons=buttons, **tracker.slots)
                    return [SlotSet(REQUESTED_SLOT, slot)]
                else:
