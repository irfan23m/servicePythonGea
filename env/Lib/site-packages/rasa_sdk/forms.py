...
from rasa_sdk.events import SlotSet, Form, EventType, FollowupAction
    ...
    def request_next_slot(
        ...
        for slot in self.required_slots(tracker):
            if self._should_request_slot(tracker, slot):
                ...
                if slot == 'kendaraan':
                    buttons = [{'title': 'mobil', 'payload': 'mobil'},{'title': 'motor', 'payload': 'motor'}]
                    dispatcher.utter_message(template='utter_ask_kendaraan', buttons=buttons, **tracker.slots)
                    ...
                    buttons = []
                    for i in result:
                        buttons.append({'title': str(i[0]), 'payload': f'brand {str(i[0])}'})
                    dispatcher.utter_message(template='utter_ask_brand', buttons=buttons, kendaraan_1=kendaraan, **tracker.slots)
                    return [SlotSet(REQUESTED_SLOT, slot)]
                elif slot == 'tipe':
                    kendaraan = tracker.get_slot("kendaraan")
                    ...
                    buttons = []
                    for i in result:
                        buttons.append({'title': str(i[0]), 'payload': f'tipe {str(i[0])}'})
                    dispatcher.utter_message(template='utter_ask_tipe', buttons=buttons, kendaraan_1=kendaraan, brand_1=brand, **tracker.slots)
                    return [SlotSet(REQUESTED_SLOT, slot)]
                elif slot == 'uang_muka':
                    ...
                    dispatcher.utter_message(template='utter_ask_uang_muka', harga=f'{harga:,}', depe_min=f'{dp_min:,}', **tracker.slots)
                    return [SlotSet(REQUESTED_SLOT, slot)]
                elif slot == 'tenor_amanah':
                    tipe = tracker.get_slot("tipe")
                    ...
                    buttons = []
                    for i in tenor_amanah:
                        buttons.append({'title': f'{i} bulan', 'payload': f'ta- {i}'})
                    dispatcher.utter_message(template='utter_ask_tenor_amanah', buttons=buttons, **tracker.slots)
                    return [SlotSet(REQUESTED_SLOT, slot)]
                elif slot == 'berat_emasku':
                    query = "SELECT berat FROM uang_muka_emasku"
                    result = select(query)
                    buttons = []
                    for i in result:
                        buttons.append({'title': f'{str(i[0])} gram', 'payload': f'b- {int(i[0])}'})
                    dispatcher.utter_message(template='utter_ask_berat_emasku', buttons=buttons, **tracker.slots)
                    return [SlotSet(REQUESTED_SLOT, slot)]
                else:
